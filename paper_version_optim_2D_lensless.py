...
from FieldGeneration import generate_target_profiles_specific_modes_2D
...
class OpticalSystem(nn.Module):
    def __init__(self, device="cpu", target_mode_nbs=(2,2), with_minimize_losses=True,downsampling=2):
        ...
        mean_wavelength = (list_wavelengths[target_mode_nbs[0]] + list_wavelengths[target_mode_nbs[1]])/2

        # Initialize components
        self.simulation = Simulation(config_dict=self.config_simulation)
        self.source = Source(config_dict=self.config_source, XY_grid=self.simulation.XY_grid,wavelength=mean_wavelength)
        ...
        self.target_modes_idx = self.target_modes[1]+self.target_modes[0]*9

        list_modes_nbs = [(i,j) for i in range(9) for j in range(9)]

        self.list_target_files = generate_target_profiles_specific_modes_2D(XY_grid=self.simulation.XY_grid,
                                                     list_modes_nbs=list_modes_nbs)
        # Move target fields to device
        self.list_target_files = [field.to(self.device) for field in self.list_target_files]
        self.target_field = self.list_target_files[self.target_modes_idx]
    ...
    def compute_loss(self, out_field, list_target_field, epoch):
        ...
        loss1 = -torch.log(list_overlaps[self.target_modes_idx] + 1e-10)

        # Exclude the target mode from the overlaps
        other_overlaps = [overlap for idx, overlap in enumerate(list_overlaps) if idx != self.target_modes_idx]
...
def optimize_phase_mask(target_mode_nbs, run_name, run_number, data_dir, num_epochs, pbar,downsampling=2):
    ...
    model = OpticalSystem(device=device, target_mode_nbs=target_mode_nbs, with_minimize_losses=True,downsampling=downsampling)
...
def run_multiple_tests(data_dir, target_modes, num_runs_per_mode, num_epochs, run_name=None,downsampling=2):
    ...
    results = []
    headers = ["Target Mode", "Run"] + [f"Mode {i,j}"  for i in range(9) for j in range(9)] + ["Inside Energy %"]
    ...
    for mode in target_modes:
        for run in range(num_runs_per_mode):
            ...
            overlaps, inside_energy = optimize_phase_mask(target_mode_nbs=mode, run_name=run_name, run_number=run + 1, data_dir=data_dir, num_epochs=num_epochs, pbar=optimize_pbar,downsampling=downsampling)
...
if __name__ == "__main__":
    ...
    run_name = "Optim_2D_lensless"  # Optional: provide a custom run name
