...
def format_val(val):
    ...
    return f'{val:.2e}' if val < 0.005 else f'{val:.2f}'
...
def main():
    ...
    dir_path = "./comparisons_results_optim_on_selectivity/"
    os.makedirs(dir_path, exist_ok=True)

    cross_correlation_matrix_davis = np.zeros((6, 6))
    cross_correlation_matrix_phase_inversion = np.zeros((6, 6))
    cross_correlation_matrix_floyd_steinberg = np.zeros((6, 6))
    cross_correlation_matrix_optim = np.zeros((6, 6))
    list_cross_correlation_matrix = [cross_correlation_matrix_davis, cross_correlation_matrix_phase_inversion, cross_correlation_matrix_floyd_steinberg, cross_correlation_matrix_optim]

    for mode_nb in range(0, 6):
        ...
        mask_4 = np.load(f"./comparisons_results_optim_on_selectivity/best_slm_phase_{mode_nb}_0.npy")
        ...
        column_titles = ["Davis et al (1999)", "Phase inversion", "Phase inversion + dithering", "Pytorch Optim. Mask"]
        ...
        for idx in range(len(cropped_amplitudes)):
            ...
            list_cross_correlation_matrix[idx][mode_nb,:] = np.array(cross_overlap_integrals[idx][0:6])
        ...
        for i in range(4):
            ...
            ax[1, i].text(0.05, 0.95, energy_text, transform=ax[1, i].transAxes, color='white', fontsize=10,
                          verticalalignment='top', bbox=dict(facecolor='black', alpha=0.5))
