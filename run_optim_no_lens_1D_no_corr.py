...
class OpticalSystem(nn.Module):
    def __init__(self, device="cpu", target_mode_nb=2, with_minimize_losses=True):
        ...
        self.list_target_files = generate_target_profiles_specific_modes(
...
def run_multiple_tests(data_dir, target_modes, num_runs_per_mode, num_epochs, run_name=None):
    ...
    results = []
    headers = ["Target Mode", "Run"] + [f"Mode {i}" for i in range(8)] + ["Inside Energy %"]
...
if __name__ == "__main__":
    ...
    run_name = "Phase_masks_lensless_final_widths_20um"  # Optional: provide a custom run name
