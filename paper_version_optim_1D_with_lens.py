...
class OpticalSystem(nn.Module):
    def __init__(self, device="cpu", target_mode_nb=2, with_minimize_losses=True, downsampling=2):
        ...
        self.source = Source(config_dict=self.config_source, XY_grid=self.simulation.XY_grid,wavelength=list_wavelengths[target_mode_nb])
        ...
        self.list_target_files = generate_target_profiles_specific_modes(XY_grid=self.ft_lens.XY_output_grid, list_modes_nb=[0,1,2,3,4,5,6,7,8])
        ...
        print(self.simulation.XY_grid[0].shape)

        self.slm = SLM(config_dict=self.config_slm, XY_grid=self.simulation.XY_grid,downsampling=downsampling)
...
def optimize_phase_mask(target_mode_nb, run_name, run_number, data_dir, num_epochs, pbar, downsampling=2):
    ...
    model = OpticalSystem(device=device, target_mode_nb=target_mode_nb, with_minimize_losses=True, downsampling=downsampling)

    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = OpticalSystem(device=device, target_mode_nb=target_mode_nb, with_minimize_losses=True, downsampling=downsampling)
    
    # Add these lines to print parameter count
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    phase_params = model.slm.phase_parameters.numel()
    print(f"Total trainable parameters: {total_params}")
    print(f"SLM phase parameters: {phase_params}")
...
def run_multiple_tests(data_dir, target_modes, num_runs_per_mode, num_epochs, run_name=None,downsampling=2):
    if run_name is None:
    ...
    for mode in target_modes:
        for run in range(num_runs_per_mode):
            ...
            overlaps, inside_energy = optimize_phase_mask(target_mode_nb=mode, 
                                                          run_name=run_name, 
                                                          run_number=run + 1, 
                                                          data_dir=data_dir, 
...
if __name__ == "__main__":
    ...
    run_name = "Optim_1D"  # Optional: provide a custom run name

    run_multiple_tests(data_dir, target_modes, num_runs_per_mode, num_epochs, run_name,downsampling=2)

    target_modes = [2,3,4]  # Add or remove modes as needed
    run_name = "Optim_1D_downsampled_factor_2"  # Optional: provide a custom run name
    run_multiple_tests(data_dir, target_modes, num_runs_per_mode, num_epochs, run_name,downsampling=4)

    run_name = "Optim_1D_downsampled_factor_4"  # Optional: provide a custom run name
    run_multiple_tests(data_dir, target_modes, num_runs_per_mode, num_epochs, run_name,downsampling=8)

    run_name = "Optim_1D_downsampled_factor_8"  # Optional: provide a custom run name
    run_multiple_tests(data_dir, target_modes, num_runs_per_mode, num_epochs, run_name,downsampling=16)
